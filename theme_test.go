...
func writeDMSFixture(t *testing.T, settingsJSON, themeJSON string) string {
	t.Helper()
	dir := t.TempDir()
...
func TestDMSAccentHex(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.settingsJSON != "" {
				settingsPath = writeDMSFixture(t, tt.settingsJSON, tt.themeJSON)
			}
			if got := dmsAccentHex(settingsPath); got != tt.want {
...
func TestCatppuccinAccentHex(t *testing.T) {
	...
	if got, want := catppuccinAccentHex("not-a-real-accent"), catppuccinAccentHex("mauve"); got != want {
...
func TestResolvePrimaryHex(t *testing.T) {
	settingsPath := filepath.Join(t.TempDir(), "does-not-exist.json")
	if got, want := resolvePrimaryHex(settingsPath, "blue"), catppuccinAccentHex("blue"); got != want {
